...
function main()
    ...
    pgfplotsx()
    x_plot = plot(t_vector, x_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"x,\ [m]", widen=true, formatter=:plain, legend = :topright)
    plot!(x_plot, t_vector, vallado_x.Column1, label = "PKEPLER routine")
    y_plot = plot(t_vector, y_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"y,\ [m]", widen=true, formatter=:plain, legend = :topright)
    plot!(y_plot, t_vector, vallado_y.Column1, label = "PKEPLER routine")
    z_plot = plot(t_vector, z_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"z,\ [m]", widen=true, formatter=:plain, legend = :topright)
    plot!(z_plot, t_vector, vallado_z.Column1, label = "PKEPLER routine")
    vx_plot = plot(t_vector, vx_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"v_{x},\ [m/s]", widen=true, formatter=:plain, legend = :topright)
    plot!(vx_plot, t_vector, vallado_vx.Column1, label = "PKEPLER routine")
    vy_plot = plot(t_vector, vy_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"v_{y},\ [m/s]", widen=true, formatter=:plain, legend = :topright)
    plot!(vy_plot, t_vector, vallado_vy.Column1, label = "PKEPLER routine")
    vz_plot = plot(t_vector, vz_dimorphos, label = "RK4 Two-Body with " * L"J2", xlabel = "Time, " * L"t,\ [s]", ylabel = L"v_{z},\ [m/s]", widen=true, formatter=:plain, legend = :topright)
