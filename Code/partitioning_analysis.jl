...
function get_intermediate_oe_and_time(a_value, e_value, i_value, Omega_value, omega_value, M_value)
    # Determine what kind of orbit we are dealing with
    global dimorphos_orbit_type = type_of_orbit(e_value, i_value)
    ...
    # Propagate Dimorphos
    x_dimorphos, y_dimorphos, z_dimorphos, vx_dimorphos, vy_dimorphos, vz_dimorphos, t_vector = runge_kutta_4(x, y, z, vx, vy, vz, mu_system, start_time, end_time, 1000, enable_perturbation)
    # Select every xth element to match the photos taken and convert to km
    photo_selector = LinRange(1, length(x_dimorphos), 1000)
    index_vector = zeros(Int64, 1000)
    ...
    intermediate_array = zeros(Float64, 4, 7)
    value_index = 1
    for i=1:4
        intermediate_array[i, 1] = t_vector[value_index]
        intermediate_array[i, 2], intermediate_array[i, 3], intermediate_array[i, 4], ~, ~, intermediate_array[i, 7] = cartesian_to_orbital_elements([x_dimorphos[value_index], y_dimorphos[value_index], z_dimorphos[value_index]], [vx_dimorphos[value_index], vy_dimorphos[value_index], vz_dimorphos[value_index]], mu_system)
        value_index = value_index + 250
    end
...
a_value = 1184.387 # Semi-major axis
e_value = 0.00000732 # Eccentricity
i_value = 0.00058 # Inclination
Omega_value = 0.0 # Longitude of the ascending node
omega_value = 0.0 # Argument of periapsis
M_value = 138.25 # True anomaly 
global start_time = 0
global end_time = 300*hour
global info_array = get_intermediate_oe_and_time(a_value, e_value, i_value, Omega_value, omega_value, M_value)

for counter=1:4
    global start_time = info_array[counter, 1]
    global end_time = info_array[counter, 1] + 75*hour 
    global a_dimorphos = info_array[counter, 2]
    global e_dimorphos = info_array[counter, 3]
    global i_dimorphos = info_array[counter, 4]
    global Omega_dimorphos = info_array[counter, 5]
    global omega_dimorphos = info_array[counter, 6]
    global M_dimorphos = info_array[counter, 7]
    global step_size = floor((end_time-start_time)/total_photos)
    global spice_start_time = utc2et("2027-02-28T08:14:58") + info_array[counter, 1]
